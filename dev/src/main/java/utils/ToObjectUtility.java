...
public class ToObjectUtility {
     ...
     */
    public static Object[][] convertAccountsToTableData(List<Person> data, String[] columnNames) {
        // Convert ArrayList<Account> to Object[][]
        Object[][] tableData = new Object[data.size()][columnNames.length];
        for (int i = 0; i < data.size(); i++) {
            Person person = data.get(i);
            tableData[i][0] = person.getIdAccount();
            tableData[i][1] = person.getFirstName();
            tableData[i][2] = person.getLastName();
            tableData[i][3] = person.getPhoneNumber();
            tableData[i][4] = person.getEmail();
            tableData[i][5] = person.getPassword();
            tableData[i][6] = person.getDomain();
