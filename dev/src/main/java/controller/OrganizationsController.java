...
import model.managementSystem.Requirement;
import model.managementSystem.Clause;
...
public class OrganizationsController {
    private static final String ORGANIZATION_FILE_PATH = FileHandler.ORGANIZATION_FILE;
    ...
    public void loadOrganizations() {
        try {
            List<Organization> loadedOrganizations = FileHandler.loadDataFromJson(ORGANIZATION_FILE_PATH, new TypeReference<List<Organization>>() {
            });
    ...
    public void saveOrganizations() {
        try {
            FileHandler.saveDataInJson(ORGANIZATION_FILE_PATH, organizations);
    ...
    public List<OrganizationProcess> getAllProcesses() {
        List<OrganizationProcess> allProcesses = new ArrayList<>();
        for (Organization organization : organizations) {
    ...
    public void createProcess(String idOrg, OrganizationProcess newProcess) throws Exception {
        Organization organization = getOrganizationById(idOrg);
    ...
    public void editOrgProcess(String idOrg, String idOrgProcess, OrganizationProcess updatedOrganizationProcess) throws Exception {
        Organization organization = getOrganizationById(idOrg);

        if (organization != null) {
            boolean isUpdated = organization.editOrgProcess(idOrgProcess, updatedOrganizationProcess);
            if (!isUpdated) {
                throw new Exception("Process with ID " + updatedOrganizationProcess + " not found.");
            }
            saveOrganizations(); // Save changes to the JSON file
