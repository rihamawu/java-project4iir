...
public class ManagementSystemManagementTabController {
    ...
    private void handleCreateManagementSystem(PopUpDialog popUpDialog) {
        if (popUpDialog.validateForm()) {
            ...
            try {
                globalControllersGetter.organizationsController.addManagementSystemToOrganization(managementSystem.getIdOrganization(), managementSystem);
                view.refreshTable();
    ...
    private void handleEditManagementSystem(PopUpDialog popUpDialog) {
        if (popUpDialog.validateForm()) {
            ...
            try {
                globalControllersGetter.organizationsController.editManagementSystemInOrganization(managementSystem.getIdOrganization(), managementSystem.getIdManagementSystem(), managementSystem);
    ...
    public void handleDeleteManagementSystem(ManagementSystemManagementTab.ButtonEditor editor) {
        ...
        if (response == JOptionPane.YES_OPTION) {
            ...
            try {
                globalControllersGetter.organizationsController.deleteManagementSystemFromOrganization(editor.getIdOrg(), editor.getIdManagementSystem());
            }catch (Exception e) {
                JOptionPane.showMessageDialog(null, e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
            view.refreshTable();
